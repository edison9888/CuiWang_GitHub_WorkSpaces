...
#define DISTNACE_TO_MAKE_MOVE_FOR_SWIPE 110
...
- (void)moveOneStep:(BOOL)isSwipingToLeftDirection {
    ...
    NSLog(@"indexOffsetFromImageLayersToTemplates == %d",indexOffsetFromImageLayersToTemplates);
...
//CALayer *targetTemplate = [self.templateLayers objectAtIndex: i + offset];
        ...
        CGFloat scale = 1.0f;
        NSLog(@"self.sideVisibleImageCount == %d",self.sideVisibleImageCount);
    ...
    if (isSwipingToLeftDirection){
        ...
        NSLog(@"self.currentRenderingImageIndex == %d    self.sideVisibleImageCount == %d",self.currentRenderingImageIndex,self.sideVisibleImageCount);
...
- (void)handleGesture:(UIPanGestureRecognizer *)recognizer {
   if (recognizer.state == UIGestureRecognizerStateChanged){
       ...
       if (abs(offset.x) > DISTNACE_TO_MAKE_MOVE_FOR_SWIPE) {
           ...
           NSLog(@"index  == %d" ,  [self getIndexForMiddle]);
       }
   }
...
+ (id)coverFlowViewWithFrame:(CGRect)frame andImages:(NSMutableArray *)rawImages sideImageCount:(int)sideCount sideImageScale:(CGFloat)sideImageScale middleImageScale:(CGFloat)middleImageScale {
    ...
    singleRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:flowView action:@selector(SingleTap:)];
    //点击的次数
...
//    singleRecognizer.numberOfTouchesRequired = 2;
    ...
    //给view添加一个手势监测；
    [flowView addGestureRecognizer:singleRecognizer];
...
-(void)SingleTap:(UITapGestureRecognizer*)recognizer
{
    ...
    if ((point.x >= 393.0 && point.x <= 645.0) && (point.y >= 199.0 && point.y <= 564.0)) {
            NSLog(@"单击  %d",[self getIndexForMiddle]);
        switch ([self getIndexForMiddle]) {
            case 0:
                [[NSNotificationCenter defaultCenter] postNotificationName:@"openWeb" object:@"http://www.tqgct.com/"];
                break;
            case 1:
                [[NSNotificationCenter defaultCenter] postNotificationName:@"openWeb" object:@"http://www.crgk.com/"];
                break;
            case 2:
                [[NSNotificationCenter defaultCenter] postNotificationName:@"openWeb" object:@"http://www.tqmpacc.com/"];
                break;
            case 3:
                [[NSNotificationCenter defaultCenter] postNotificationName:@"openWeb" object:@"http://www.tqmba.com/index.html"];
                break;
            case 4:
                [[NSNotificationCenter defaultCenter] postNotificationName:@"openWeb" object:@"http://www.tqkaoyan.com/"];
                break;
            case 5:
                [[NSNotificationCenter defaultCenter] postNotificationName:@"openWeb" object:@"http://www.xhcedu.com/"];
                break;
            default:
...
- (int)getIndexForMiddle {
    ...
    return _currentRenderingImageIndex;
}
