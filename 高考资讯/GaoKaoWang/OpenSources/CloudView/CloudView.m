...
@synthesize cloudDelegate;
//点击标签
-(void)touchUpInside:(UILabel *)lb{
    [self.cloudDelegate touchUpInsideDelegate:lb];
}
//点下标签暂停动画
-(void)touchDown:(UILabel *)ges{
    CFTimeInterval pausedTime = [ges.layer convertTime:CACurrentMediaTime() fromLayer:nil];
    ges.layer.speed = 0.0;
    ges.layer.timeOffset = pausedTime;
}
//点离标签继续动画
-(void)touchOut:(UILabel *)but{
...
//    ++tempIndex    ;
    ...
    CGFloat width2=[(NSString *)ob2 boundingRectWithSize:CGSizeMake(1000, FONTHEIGHT) options:(NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} context:nil].size.width;
...
//    CGFloat width3=[(NSString *)ob3 sizeWithFont:[UIFont systemFontOfSize:16] constrainedToSize:CGSizeMake(1000, FONTHEIGHT)].width;
    CGFloat width3=[(NSString *)ob3 boundingRectWithSize:CGSizeMake(1000, FONTHEIGHT) options:(NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} context:nil].size.width;
...
//        CGFloat thstrwidth=[thstr sizeWithFont:thfont constrainedToSize:CGSizeMake(1000, FONTHEIGHT)].width;
        ...
        CGFloat thstrwidth=[thstr boundingRectWithSize:CGSizeMake(1000, FONTHEIGHT) options:(NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName:thfont} context:nil].size.width;
        ...
        basicScale.removedOnCompletion=YES;
        ...
        basicOpa.removedOnCompletion=YES;
        ...
        keyPosi.removedOnCompletion=YES;
...
-(void)removeAllView
{
    if (butAry) {
        if (butAry.count > 0) {
            for (BubbleV *bc in butAry) {
                
                [bc.layer removeAllAnimations];
                [bc release];
            }
        }
    }
