...
- (void)reloadData
{
    ...
    {        
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            dataInternal.data = (NSBubbleData *)bubbleData[i];
            
            // Calculating cell height
        
//            dataInternal.labelSize = [(dataInternal.data.text ? dataInternal.data.text : @"") sizeWithFont:[UIFont systemFontOfSize:22] constrainedToSize:CGSizeMake(220, 9999) lineBreakMode:NSLineBreakByWordWrapping];
        dataInternal.labelSize = [(dataInternal.data.text ? dataInternal.data.text : @"") boundingRectWithSize:CGSizeMake(220, 9999) options:(NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:22]} context:nil].size;
            ...
            if ([dataInternal.data.date timeIntervalSinceDate:last] > self.snapInterval)
            {
                ...
                (self.bubbleDictionary)[[NSString stringWithFormat:@"%d",i]] = currentSection;
...
- (float)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSBubbleDataInternal *dataInternal = ((NSBubbleDataInternal *)(self.bubbleDictionary)[key][indexPath.row]);
